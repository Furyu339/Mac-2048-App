...
struct GameLogic {
    static let size = 4
    static let boardCount = size * size
    ...
    private static func slideAndMerge(_ values: [Int], indices: [Int]) -> ([Int], Int, [Int], [Movement]) {
        ...
        while i < tiles.count {
            if i + 1 < tiles.count, tiles[i].value == tiles[i + 1].value {
                ...
                lineMoves.append(Movement(fromIndex: tiles[i].index, toIndex: destIndex, value: tiles[i].value, isMerge: true))
                lineMoves.append(Movement(fromIndex: tiles[i + 1].index, toIndex: destIndex, value: tiles[i + 1].value, isMerge: true))
                i += 2
            } else {
                result.append(tiles[i].value)
                let destPos = result.count - 1
                let destIndex = indices[destPos]
                lineMoves.append(Movement(fromIndex: tiles[i].index, toIndex: destIndex, value: tiles[i].value, isMerge: false))
                i += 1
            }
        }
