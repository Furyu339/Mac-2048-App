...
final class GameScene: SKScene {
    ...
    func playMove(
        ...
        let movingFrom = Set(movements.map { $0.fromIndex })
        ...
        for move in movements {
            ...
            tile.position = point(for: move.fromIndex)
            overlayLayer.addChild(tile)

            let moveAction = SKAction.move(to: point(for: move.toIndex), duration: moveDuration)
    ...
    private func layoutBoard() {
        ...
        let cell = cellSize()
        for row in 0..<GameLogic.size {
            for col in 0..<GameLogic.size {
    ...
    private func point(for index: Int) -> CGPoint {
        let row = index / GameLogic.size
        let col = index % GameLogic.size
        let cell = cellSize()
        let x = padding + CGFloat(col) * (cell.width + spacing) + cell.width / 2
        let y = padding + CGFloat(GameLogic.size - 1 - row) * (cell.height + spacing) + cell.height / 2
