...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 22) {
                header
                scoreRow
                BoardSpriteView(model: vm.model)
                    .frame(maxWidth: 520)
    ...
    private var header: some View {
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 6) {
                Text(vm.model.isGameOver ? "游戏结束" : "状态正常")
                    .font(.system(size: 14, weight: .semibold, design: .rounded))
                    .foregroundColor(vm.model.isGameOver ? .red.opacity(0.9) : .white.opacity(0.7))
                Text("目标: 2048+")
    ...
    private var scoreRow: some View {
        HStack(spacing: 14) {
            scoreCard(title: "当前分数", value: "\(vm.model.score)")
            scoreCard(title: "最高分", value: "\(vm.model.bestScore)")
            hintCard
        }
